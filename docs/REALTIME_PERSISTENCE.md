...
- **Як читати логи (live updates):** Backend: `[WS] connected`, `[WS] join_doc room=sheet:N`, `[WS] op_in docId/room/socketId/userId/opType`, `[WS] op_out broadcast room=... recipientsCount=N`. Якщо два клієнти в одному doc — має бути 2× join_doc для sheet:docId і recipientsCount≥1 при op_out. Frontend: `[collab] op_out` при відправці op, `[collab] op_in(remote)` при отриманні OP_APPLIED; `[sheetCollab] connected=true/false` при зміні режиму. Якщо клієнт #2 не бачить зміни без F5 — перевірити, що на сервері є два socketId в room і op_out показує recipientsCount≥1.
