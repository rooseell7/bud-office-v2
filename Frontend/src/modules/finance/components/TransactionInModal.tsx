...
export const TransactionInModal: React.FC<Props> = ({ open, onClose, onSubmit, projectId }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [w, c] = await Promise.all([getWallets(), getCategories('in')]);
        setWallets(w);
        setCategories(c);
        if (w.length > 0 && form.walletId === 0) {
          setForm((f) => ({ ...f, walletId: w[0].id, currency: w[0].currency }));
        }
        if (c.length > 0 && form.categoryId === 0) {
          const def = c.find((x) => x.name.includes('Оплата')) ?? c[0];
          setForm((f) => ({ ...f, categoryId: def.id }));
        }
      } catch {
        setWallets([]);
        setCategories([]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>Отримали гроші</DialogTitle>
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 0.5 }}>
          ...
          <TextField label="Контрагент (замовник)" value={form.counterparty} onChange={(e) => setForm((f) => ({ ...f, counterparty: e.target.value }))} fullWidth size="small" />
