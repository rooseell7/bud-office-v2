  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        { to: '/estimate/acts/intake', label: 'Прийом актів', icon: <AssignmentOutlinedIcon />, permission: 'acts:finalize' },
    ...
    },
    {
      ...
      items: [
        { to: '/execution/projects', label: "Об'єкти", icon: <ConstructionOutlinedIcon /> },
        { to: '/realization/acts/approval', label: 'Погодження актів', icon: <AssignmentOutlinedIcon />, permission: 'acts:approve' },
      ],
    },
    {
      title: 'Відділ постачання',
      items: [
        { to: '/invoices', label: 'Накладні', icon: <ReceiptLongOutlinedIcon /> },
    ...
    },
    {
      ...
      items: [
        ...
        { to: '/foreman/acts/submit', label: 'Подача актів', icon: <SendOutlinedIcon />, permission: ['acts:read', 'acts:submit'] },
      ],
    },
    {
        ...
        >
          {navGroups
            ...
            <Box key={group.title} sx={{ mt: idx === 0 ? 0 : undefined }}>
              ...
              {group.items
                .filter((item) => {
                  const p = (item as { permission?: string | string[] }).permission;
                  if (!p) return true;
                  if (Array.isArray(p)) return p.every((code) => can(code));
                  return can(p);
                })
                .map((item) => (
                <ListItemButton
