...
### 0.1. Встановити залежності Backend (перед першим запуском)

Якщо Backend ще не запускався або після `git clone`:
...
Після цього можна запускати `npm run start` або `npm run start:dev`.
